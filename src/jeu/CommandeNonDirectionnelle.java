...
public enum CommandeNonDirectionnelle implements Commande {
	...
	PRENDRE("P", "P <objet> (prendre un objet visible)"),
	INVENTAIRE("I", "I (voir l'inventaire)"), OUVRIR("OU", "OU <nom> (ouvrir un conteneur ou passage)"),
	ALLUMER_FEU("AF", "AF (allumer la cheminee)"), ALLUMER_FEU_2("CMD2", "CMD2 (essayer la cheminee)"),
	ALLUMER_LUM("CMD1", "CMD1 (allumer la lumiere)"), BRULER("B", "B (bruler les fragments)"),
	REPONDRE("REP", "REPONDRE <texte> (repondre a une enigme)"), SAUVER("SAUV", "SAUV (sauvegarder la partie)"),
	ABANDON("AB", "AB (abandonner la partie)"), TEST("T", "T (executer une partie de test)"),
	MIROIR("M", "M (activer le miroir)"), TELEPORTER("TP", "TP <piece> (teleportation via miroir)");
